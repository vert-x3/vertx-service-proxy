...
[source,groovy,subs="+attributes"]
----
compileOnly 'io.vertx:vertx-codegen:4.0.0-SNAPSHOT:processor'
----
* Gradle >= 5 (in your `build.gradle` file):
...
`vertx-lang-groovy` for Groovy or `vertx-rx-java2-gen` for RxJava 2
...
[source]
----
compile "io.vertx:vertx-codegen:4.0.0-SNAPSHOT:processor"
compile "io.vertx:vertx-service-proxy:4.0.0-SNAPSHOT"
----
...
* Any class that provides a `JsonCodec` registered in `@ModuleGen` annotation
...
To use a type of your choice freely in your service proxy, you must provide a way to encode and decode it to a Json type.
You must implement the {@link io.vertx.core.spi.json.JsonCodec} interface and then register it to the {@link io.vertx.core.spi.json.ModuleGen} annotation, like:
