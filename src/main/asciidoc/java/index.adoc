...
* Any class that provides a `JsonCodec`
...
If you want to define a custom way to encode and decode types to Json, you can define a {@link io.vertx.core.spi.json.JsonCodec} for it.
...
After you define the {@link io.vertx.core.spi.json.JsonCodec}, you must add it tp `@ModuleGen` annotation in your `package-info.java`, e.g.:

[source,java]
----
@ModuleGen(
  name = "mypackage",
  groupPackage = "io.something",
  codecs = [
