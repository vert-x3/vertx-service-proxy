...
Touse* Vert.x Service Proxies, add the following dependency to the _dependencies_ section of
...
Toimplement* service proxies, also add:
...
bus, and it will also generate aclient side proxy* for your service, so your clients can use a rich idiomatic
...
available from any host. If you do not want your service to be accessible to other nodes in the cluster, use the
`link:../../apidocs/io/vertx/serviceproxy/ServiceBinder.html#registerLocal-java.lang.Class-T-[registerLocal]` method:

[source, java]
----
SomeDatabaseService service = new SomeDatabaseServiceImpl();
// Register the handler locally (will not be propagated to other nodes in the cluster)
new ServiceBinder(vertx)
  .setAddress("database-service-address")
  .registerLocal(SomeDatabaseService.class, service);
----
...
Of course, you don'thave to* use client proxies to access remote service if you don't want to. It's perfectly acceptable
to interact with them by just sending messages over the event bus.

In order for services to be interacted with a consistent way the following message formatsmust be used* for any
Vert.x services.
