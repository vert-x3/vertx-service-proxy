...
public class ServiceProxyHandlerGen extends Generator<ProxyModel> {
  ...
  public String render(ProxyModel model, int index, int size, Map<String, Object> session) {
    ...
    replaceConstructorsPlaceholders(model, proxyHandlerClassTemplate);
    replaceServiceFieldPlaceholder(model, proxyHandlerClassTemplate);
  ...
  private void replaceConstructorsPlaceholders(ProxyModel model, ClassOrInterfaceDeclaration template) {
    template.findAll(ConstructorDeclaration.class)
      .forEach(c -> {
        c.setName(className(model));
        c.getParameterByName("service").ifPresent(it -> it.setType(model.getIfaceSimpleName()));
      });
  }

  private void replaceServiceFieldPlaceholder(ProxyModel model, ClassOrInterfaceDeclaration template) {
    template.findAll(FieldDeclaration.class, type -> "$ServiceName$".equals(type.getElementType().asString()))
      .forEach(type -> type.setAllTypes(parseType(model.getIfaceSimpleName())));
  }
  ...
  public String generateJsonParamExtract(ParamInfo param) {
    ...
    if (isCharacter(typeName)) {
      return charParamExtract(name);
    }
    if (isIntegerNumberType(typeName)) {
      return integerParamExtract(name, typeName);
    }
    if (isFloatingNumberType(typeName)) {
      return floatingParamExtract(name, typeName);
    }
    if (type.getKind() == ClassKind.ENUM) {
      return enumParamExtract(param, name);
    }
    if (type.getKind() == ClassKind.LIST || type.getKind() == ClassKind.SET) {
      return listOrSetParamExtract(name, type);
    }
    if (type.getKind() == ClassKind.MAP) {
      return mapParamExtract(name, type);
    }
  ...
  public String generateHandler(TypeInfo typeArg) {
    if (typeArg.getKind() == ClassKind.LIST || typeArg.getKind() == ClassKind.SET) {
      return listOrSetTypeHandler(typeArg);
    }
    if (typeArg.getKind() == ClassKind.MAP) {
      return mapTypeHandler((ParameterizedTypeInfo) typeArg);
    }
    if (typeArg.isDataObjectHolder()) {
      return dataObjectTypeHandler((ClassTypeInfo) typeArg);
    }
  ...
  private String mapParamExtract(String name, TypeInfo type) {
    ...
    if (isBoxedNumericType(typeArg.getName())) {
      return utils.generatedCodeBlock("json.getJsonObject($S).getMap().entrySet().stream()"
    ...
    if (typeArg.isDataObjectHolder()) {
      ...
      return utils.generatedCodeBlock("json.getJsonObject($S).stream().collect($T.toMap($T.Entry::getKey, e -> $L))",
  ...
  private String listOrSetParamExtract(String name, TypeInfo type) {
    ...
    if (typeArg.isDataObjectHolder()) {
      ...
      return utils.generatedCodeBlock(
        "json.getJsonArray($S).stream().map(v -> $L).collect($T.to$L())",
        name, GeneratorUtils.generateDeserializeDataObject("v", doType), Collectors.class, coll);
    }
    if (isBoxedIntegerType(typeArg.getName())) {
      return utils.generatedCodeBlock("json.getJsonArray($S).stream().map(o -> ((Number)o).$LValue()).collect($T.to$L())",
  ...
  private String apiTypeHandler(TypeInfo typeArg) {
    return utils.generatedCodeBlock(utils.proxyHandlerHandleMethodApiTypeHandle,
  ...
  private String dataObjectTypeHandler(ClassTypeInfo typeArg) {
    return utils.generatedCodeBlock(utils.proxyHandlerHandleMethodDataObjectTypeHandle,
  ...
  private String mapTypeHandler(ParameterizedTypeInfo typeArg) {
    ...
    if (innerTypeArg.isDataObjectHolder()) {
      return utils.generatedCodeBlock(utils.proxyHandlerHandleMethodMapTypeHandle,
  ...
  private String listOrSetTypeHandler(TypeInfo typeArg) {
    ...
    if (innerTypeArg.isDataObjectHolder()) {
      return utils.generatedCodeBlock(utils.proxyHandlerHandleMethodListOrSetTypeHandle,
